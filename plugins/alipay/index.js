...
const createOrder = async (user, account, amount, orderType = 3) => {
  ...
  const addr = jsjConfig.jsjGatewayUrl + '&addnum=' + orderId + '&total=' + amount + '&apiid=' + jsjConfig.apiid + '&apikey=' + md5(jsjConfig.apikey) + '&showurl=' + config.plugins.webgui.site + '/api/user/alipay/callback%3fc';
  logger.info(addr);
